...
-- FOREIGN KEY (외래키컬럼) REFERENCE 참조테이블(기본키);

-- 2) 외래키 : MS_BOARD(WRITER) ----> MS_USER(USER_no)
...
INSERT INTO MS_FILE VALUES(3,1,'고양이.webp', '123', '2024/09/05','2024/09/05', 'webp');
...
-- - COMMIT, ROLLBACK 을 이용하여 변경사항을 적용하거나
--   되돌릴 수 있음.
...
DECLARE
    ...
    VS_EMP_NAME EMPOYEE.EMP_NAME%TYPE;
...
BEGIN
        ...
        ELSEIF V_YEAR_OF_WORK > 13 THEN V_JOB_CODE := 'J3';
        ELSEIF V_YEAR_OF_WORK > 10 THEN V_JOB_CODE := 'J4';
        ELSEIF V_YEAR_OF_WORK > 6 THEN V_JOB_CODE := 'J5';
        ELSEIF V_YEAR_OF_WORK > 3 THEN V_JOB_CODE := 'J6';
        ELSE
        ...
        -- 결과 메세지 출력
        DBMS_OUTPUT.PUT_LINE(p_emp_id || ' 사원을 ' || V_JOB_CODE || '직급으로 승진시켰습니다.');
